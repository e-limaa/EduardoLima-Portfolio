...
const resolveColorStop = (scope: HTMLElement, value: string, fallback: string) => {
  ...
  probe.remove();
  return resolved === "rgba(0, 0, 0, 0)" ? fallback : resolved;
};
  ...
  useEffect(() => {
    ...
    const resolvedColors = colors.map((color, index) =>
      resolveColorStop(
        ...
        color,
        index === 0 ? "#5227FF" : index === 1 ? "#7CAFFF" : "#B19EEF"
      )
    );
...
`;
  ...
  return (
    <div
      ref={mountRef}
      className={cn(
        "liquid-ether-container relative h-full w-full overflow-hidden pointer-events-none touch-none",
        className
      )}
      style={style}
    />
