...
import { InteractiveGrid, Button, SectionHeader } from "@limia/design-system";
import { ContactFormModal } from "./ContactFormModal";
import { useLanguage } from "../language-provider";

export const CTA = () => {
  const { t } = useLanguage();
  const whatsappNumber = import.meta.env.VITE_WHATSAPP_NUMBER?.replace(/\D/g, "").trim();
  const whatsappUrl = whatsappNumber ? `https://wa.me/${whatsappNumber}` : undefined;
  const linkedinUrl = import.meta.env.VITE_LINKEDIN_URL?.trim() || "https://linkedin.com";
